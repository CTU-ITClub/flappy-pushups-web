...
class Bird {
    ...
    update(faceData, screenWidth, screenHeight) {
        if (faceData && faceData.faceDetected) {
            ...
            screenX = Math.max(this.width / 2, Math.min(screenWidth - this.width / 2, screenX));
            screenY = Math.max(this.height / 2, Math.min(screenHeight - this.height / 2, screenY));
        ...
        if (Math.abs(this.targetX - this.x) > this.deadZone) {
            ...
            const moveAmount = Math.min(Math.abs(this.targetX - this.x), this.maxSpeed);
            ...
            this.x = (this.xHistory[0] + 2*this.xHistory[1] + 3*this.xHistory[2] + 4*this.xHistory[3] + 5*this.xHistory[4]) / 15;
        }
        ...
        if (Math.abs(this.targetY - this.y) > this.deadZone) {
            ...
            const moveAmount = Math.min(Math.abs(this.targetY - this.y), this.maxSpeed);
            const newY = this.y + direction * moveAmount;
            
            this.yHistory.shift();
            this.yHistory.push(newY);
            
            this.y = (this.yHistory[0] + 2*this.yHistory[1] + 3*this.yHistory[2] + 4*this.yHistory[3] + 5*this.yHistory[4]) / 15;
        }
        ...
        if (this.rotation < this.targetRotation) {
            this.rotation = Math.min(this.rotation + rotationSpeed, this.targetRotation);
        } else if (this.rotation > this.targetRotation) {
            this.rotation = Math.max(this.rotation - rotationSpeed, this.targetRotation);
        }
